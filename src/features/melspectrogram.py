...
def instanciate_mel_spectrogram(params):
    mel_spectrogram_no_pad = torchaudio.transforms.MelSpectrogram(
        ...
        norm=params.get("norm", "slaney"),
        mel_scale=params.get("mel_scale", "slaney")
    )
    if params.get("use_db", False):
        amplitude_to_db = torchaudio.transforms.AmplitudeToDB(stype="power", top_db=80)
        mel_spectrogram_no_pad = torch.nn.Sequential(mel_spectrogram_no_pad, amplitude_to_db)
...
def get_embeddings(alignment_df, params):
    ...
    extractor = instanciate_mel_spectrogram(params)
